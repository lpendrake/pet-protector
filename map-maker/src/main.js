...
import { ToolPickerPanel } from './ui/ToolPickerPanel.js';
import { PalettePanel } from './ui/PalettePanel.js';
import { MapsManagerPanel } from './ui/MapsManagerPanel.js';
...
const atlas = new SpriteAtlas(tiles, items);
const viewportContainer = document.getElementById('viewport-container');
const renderer = new MapRenderer(viewportContainer, bus, state, items, atlas);
...
const propertyPanel = new PropertyPanel(bus, state, viewportContainer);
const toolbar = new Toolbar(bus, viewportContainer);
const toolPicker = new ToolPickerPanel(bus, viewportContainer);
const palette = new PalettePanel(bus, tiles, items, viewportContainer, atlas);
const mapsManager = new MapsManagerPanel(bus, persistence, viewportContainer);

toolbar.wireHoverPanel('tool', toolPicker);
toolbar.wireHoverPanel('placeables', palette);
toolbar.setEntityState(state);
toolbar.wireTogglePanel('maps', mapsManager);

async function start() {
    try {
        await atlas.load();
        palette.refresh(); // rebuild palette now that sprite sheets are loaded
        await renderer.init();
    } catch (e) {
